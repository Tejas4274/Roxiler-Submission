...
app.get("/api/bar-chart", async (req, res) => {
  ...
  if (!month || month < 1 || month > 12) {
    return res
      .status(400)
      .json({
        error: "Month parameter is required and must be between 1 and 12",
      });
  }
  ...
  try {
    ...
    const data = await Promise.all(
      ranges.map(async (range) => {
        ...
        console.log(
          `Query for range ${range.label}: price >= ${range.min} && price < ${range.max} && month matches ${month}`
        );
        console.log(`Count for ${range.label}:`, count);
